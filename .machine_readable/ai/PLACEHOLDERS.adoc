...
All placeholders in this template follow the `` pattern.
...
sed -i 's/Patch Bridge/my-project/g' $(grep -rl 'Patch Bridge' .)
sed -i 's/PATCH_BRIDGE/MY_PROJECT/g' $(grep -rl 'PATCH_BRIDGE' .)
sed -i 's/{{project}}/my_project/g' $(grep -rl '{{project}}' .)
sed -i 's/patch-bridge/my-project/g' $(grep -rl 'patch-bridge' .)
sed -i 's/{{FORGE}}/github.com/g' $(grep -rl '{{FORGE}}' .)
sed -i "s/2026/$(date +%Y)/g" $(grep -rl '2026' .)
sed -i "s/2026-04-13/$(date +%Y-%m-%d)/g" $(grep -rl '2026-04-13' .)
```
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Patch Bridge` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `CVE-mitigation design and extraction target from panic-attacker` | One-line description | `A tool for X` | flake.nix |
| `PATCH_BRIDGE` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `patch-bridge` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `2026-04-13` | Current date (ISO) | `2026-02-14` | STATE.a2ml, MAINTAINERS.md |
...
These are **not** standard `` markers -- they are TODO markers
